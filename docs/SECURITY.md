...
- Sidebar hiding is not security by itself; server-side module route blocking is currently enforced for `articles`, `customers`, and `suppliers`.
- Shared routes and finance/order/stock/report modules are not route-blocked until reviewed and wired module-by-module.
